...
for k in range(len(ua)):
    Y.append(str(ua[k]) + "," + str(ui[k]))


data = pd.DataFrame(index = Y)

names = ["num_spots","total_coverage","avg_spot_size","spot_size_std","avg_circ","circ_std"]
categories = [num_spots,total_coverage,avg_spot_size,spot_size_std,avg_circ,circ_std]
